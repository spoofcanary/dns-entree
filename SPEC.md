...
A Go library, HTTP API, and embeddable JS widget that lets any SaaS product push DNS records to a customer's domain. Detect the DNS provider, authenticate, push records, verify. Four steps, one library, zero vendor lock-in.
...
**Widget mode.** Embed the `@dns-entree/widget` JS widget in any web page. Three auto-selected tiers: Domain Connect (zero credentials), API credentials (provider token), copy-paste fallback. Shadow DOM isolated, light/dark theming, under 20 KB gzipped. Ships via npm and CDN. Talks to the API server for backend operations.
