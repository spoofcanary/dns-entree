...
func TestRun_GCOnce_SweepsExpired(t *testing.T) {
	dir := t.TempDir()
	...
	now := time.Now()
	expired := &migrate.StoredMigration{
	...
	code := run(context.Background(), []string{"gc", "--once", "--state-dir", dir}, func(string) string { return "" }, &buf)
	if code != 0 {
		t.Fatalf("gc --once exit code=%d, want 0; stderr=%q", code, buf.String())
	}
	if !strings.Contains(buf.String(), "swept 1 expired migrations") {
		t.Errorf("unexpected output: %q", buf.String())
	}

	if _, err := store.Get(ctx, expired.ID); !errors.Is(err, migrate.ErrNotFound) {
		t.Errorf("expired row still present, err=%v", err)
	}
	if _, err := store.Get(ctx, valid.ID); err != nil {
		t.Errorf("valid row removed: %v", err)
	}
}

func TestRun_GCOnce_RequiresOnce(t *testing.T) {
	var buf bytes.Buffer
	code := run(context.Background(), []string{"gc", "--state-dir", t.TempDir()}, func(string) string { return "" }, &buf)
	if code == 0 {
		t.Errorf("expected non-zero exit when --once missing")
	}
}
